...
<div class="row" style="justify-content:space-between">
  ...
  <div class="row"><button onclick={() => { jahr--; load(); }}>‹</button><strong>{jahr}</strong><button onclick={() => { jahr++; load(); }}>›</button></div>
</div>
      ...
      <tbody>
        {#each list as h}
          <tr><td class="mono">{dateDe(h.datum)}</td><td>{h.name}{#if h.betrieblich} <span class="badge info">betrieblich</span>{/if}</td>
            <td>{#if h.betrieblich}<button class="small" onclick={() => del(h.datum)}>Entfernen</button>{/if}</td></tr>
        {/each}
      </tbody>
    </table>
  </div>
  <div class="card">
    <h3 style="margin-top:0">Betrieblichen freien Tag hinzufügen</h3>
