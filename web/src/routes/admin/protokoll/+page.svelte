...
<div class="page-head"><h1>Protokoll</h1></div>
    ...
    <tbody>
      {#each rows as r}
        <tr onclick={() => (open = open === r.id ? null : r.id)} style="cursor:pointer">
          <td class="muted">{fmt(r.ts)}</td><td>{r.actor}</td><td>{r.aktion}</td><td class="small muted">{r.ziel ?? ''}</td>
        </tr>
        {#if open === r.id}
          <tr><td colspan="4" style="height:auto;padding:12px 16px;background:var(--surface-2)"><pre class="small" style="white-space:pre-wrap;margin:0">{JSON.stringify({ vorher: r.vorher, nachher: r.nachher }, null, 2)}</pre></td></tr>
        {/if}
      {/each}
    </tbody>
  </table>
